...
namespace DomainAbstractions
{
    public static class TaskMonadExtensionMethods
    {
        ...
        public static Continuation<T,U> Bind<T, U>(this IContinuable<T> source, Func<T, Task<U>> function)
        {
            return (Continuation<T, U>)source.WireIn(new Continuation<T, U>(function));
        }
...
namespace ProgrammingParadigms
{
    ...
    // The IContuable interface is only used as the type for the this parameter of the Bind function.
    // Put it on all domain abstraction classes that are sources, that is can be wired to a Continuation class using Bind.
    // The IContinuation in terface is the one actually used to implement the Continuation dataflow programming paradigm
