...
#define ALAVersionBind                  // ALA Version uses domain abstractions but uses a Bind function to have identical application layer code
...
// Notice how it indents for every ContinueWith, which is not practical (leads triangle hell for longer chains of functions)
        ...
        // It uses Unwrap which avoids indenting for every new continuation
        static void ComposedFunction()
        {
#if DebugThreads
            Console.WriteLine($"Console thread: {Thread.CurrentThread.ManagedThreadId}");
        ...
        static void ComposedFunction()
        {
            ...
            .WireIn(new ContinuationAction<int>((x) => { Console.WriteLine($"Final result is {x}."); }));
            program.Run();
        }
#endif
